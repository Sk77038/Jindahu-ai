...
export default function App() {
  ...
  // Load User and Detect "First Unlock of the Day"
  useEffect(() => {
    ...
    if (profile) {
      ...
      // --- Simulation of "Unlock Phone Trigger" ---
      const lastCheckInDate = new Date(profile.lastCheckIn).toDateString();
      const todayDate = new Date().toDateString();
      const currentHour = new Date().getHours();

      // If it's a new day and it's morning (6 AM - 12 PM)
  ...
  if (!user) return (
    <div className="max-w-md mx-auto h-screen bg-white p-8">
      <h1 className="text-3xl font-black mb-6">Real-time Shield</h1>
      <input id="setup-name" type="text" placeholder="Name" className="w-full p-4 bg-slate-100 rounded-2xl mb-4" />
      <input id="setup-phone" type="tel" placeholder="Phone" className="w-full p-4 bg-slate-100 rounded-2xl mb-4" />
      <button 
        onClick={() => {
          ...
          if (name && phone) {
            const u: UserProfile = { name, phone, checkInHour: 9, lastCheckIn: Date.now(), contacts: [], language: Language.EN };
            setUser(u);
  ...
  return (
    <div className="max-w-md mx-auto min-h-screen bg-slate-50 flex flex-col relative overflow-hidden">
      {/* Forced Morning Check-in Modal */}
      {showMorningForce && (
        <div className="fixed inset-0 bg-yellow-400 z-[500] flex flex-col items-center justify-center p-8 text-center animate-fade-in">
           <div className="mb-6"><ICONS.Sun /></div>
           <h1 className="text-4xl font-black text-slate-900 mb-2">{t.goodMorning}</h1>
           <p className="text-xl font-bold text-slate-700 mb-12 opacity-80">{t.morningPrompt}</p>
           
           <button 
             onClick={handleCheckIn}
             className="w-72 h-72 rounded-full bg-white text-green-600 shadow-2xl flex flex-col items-center justify-center p-8 border-[12px] border-green-100 active:scale-95 transition-all"
           >
              <span className="text-3xl font-black">{t.imAlive}</span>
              <div className="mt-4 w-4 h-4 bg-green-500 rounded-full animate-ping"></div>
           </button>
           
           <p className="mt-12 text-sm font-bold text-yellow-900 uppercase tracking-widest">Auto-launch triggered via unlock</p>
        </div>
      )}
      ...
      <header className="bg-white p-5 flex justify-between items-center border-b sticky top-0 z-50">
        ...
        <div className="flex gap-2">
           <button onClick={() => setLanguage(l => l === Language.EN ? Language.HI : Language.EN)} className="text-[10px] font-black bg-slate-100 px-2 py-1 rounded">
             {language === Language.EN ? 'HI' : 'EN'}
           </button>
           {batteryLevel !== null && <span className="text-[10px] font-bold px-2 py-1 bg-slate-100 rounded">⚡ {batteryLevel}%</span>}
        </div>
      </header>
      ...
      <main className="flex-grow p-6 space-y-6">
        <div className={`p-6 rounded-[2.5rem] shadow-xl border-b-8 transition-all ${
          ...
          <h3 className="text-3xl font-black mb-4">{status === SafetyStatus.SAFE ? t.safe : t.emergency}</h3>
          <div className="bg-white/10 rounded-2xl p-4 backdrop-blur-md">
            <p className="text-[10px] font-bold uppercase opacity-70">Next Check-in in</p>
      ...
      <div className="p-6 pb-10 bg-white border-t sticky bottom-0">
        <button onClick={() => setShowPanicConfirm(true)} className="w-full bg-red-600 text-white py-5 rounded-3xl font-black text-xl">
          {t.panic}
        </button>
      </div>
