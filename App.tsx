...
export default function App() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      if (diff <= 0) { setStatus(SafetyStatus.ATTENTION); setTimeRemaining('00:00:00'); }
      else {
  ...
  const startScan = (isDaily: boolean) => {
    ...
    scanTimerRef.current = setInterval(() => {
      setScanProgress(p => {
        if (p >= 100) { clearInterval(scanTimerRef.current!); isDaily ? finishCheckIn() : finishRegistration(); return 100; }
  ...
  const finishRegistration = async () => {
    ...
    const newUser: UserProfile = {
      ...
      registrationDate: Date.now(), initialSoulAge: res.soulAge || "Strong Spirit",
  ...
  const handleDostChat = async () => {
    ...
    const msg = chatInput; setChatMessages(p => [...p, { role: 'user', text: msg }]);
    ...
    <div className={`max-w-md mx-auto min-h-screen bg-slate-50 flex flex-col relative overflow-hidden ${status === SafetyStatus.EMERGENCY ? 'sos-active-bg' : ''}`}>
      ...
      <main className="flex-grow p-6 space-y-6 overflow-y-auto">
        ...
        <div className="grid grid-cols-2 gap-4">
           ...
           <button onClick={() => { setShowDostAi(true); setChatMessages([{role:'ai', text: t.findingHospitals}]); }} className="bg-white p-6 rounded-[2.5rem] border flex flex-col items-center gap-2 shadow-sm active:scale-95">
      ...
      <footer className="p-6 pb-12 bg-white border-t sticky bottom-0 z-[100]">
         <button onClick={() => setStatus(SafetyStatus.EMERGENCY)} className="w-full bg-red-600 text-white py-8 rounded-[3rem] font-black text-4xl shadow-2xl active:scale-95 uppercase">{t.panic}</button>
      </footer>
      ...
      {showDostAi && (
        <div className="fixed inset-0 z-[10000] bg-slate-950 flex flex-col animate-fade-in">
           <header className="p-6 bg-slate-900 text-white flex justify-between items-center border-b border-white/5">
              <div className="flex items-center gap-3"><ICONS.Robot /><h2 className="text-xl font-black uppercase">{t.dostAi}</h2></div>
              <button onClick={() => setShowDostAi(false)} className="p-3 bg-white/5 rounded-full"><ICONS.Close /></button>
           </header>
           <div className="flex-grow overflow-y-auto p-6 space-y-4">
              {chatMessages.map((m, i) => (
                <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'} animate-slide-up`}>
                   <div className={`max-w-[85%] p-5 rounded-[2.5rem] font-bold text-sm shadow-xl ${m.role === 'user' ? 'bg-purple-600 text-white rounded-tr-none' : 'bg-slate-900 text-slate-200 border rounded-tl-none'}`}>{m.text}</div>
                </div>
              ))}
           </div>
           <div className="p-6 bg-slate-900 border-t border-white/5 flex gap-2 pb-12 shadow-2xl">
              <input value={chatInput} onChange={e => setChatInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleDostChat()} placeholder="Ask me anything..." className="flex-grow bg-white/5 p-5 rounded-3xl text-white font-bold outline-none" />
              <button onClick={handleDostChat} className="p-5 bg-purple-600 text-white rounded-3xl active:scale-90"><ICONS.Send /></button>
           </div>
        </div>
      )}
      ...
      {showSettings && (
        <div className="fixed inset-0 z-[15000] bg-white flex flex-col animate-slide-up">
          <header className="p-6 border-b flex justify-between items-center bg-slate-50">
            ...
            <button onClick={() => setShowSettings(false)} className="p-3 bg-white border rounded-full"><ICONS.Close /></button>
          </header>
          <div className="p-8 space-y-6">
             <div className="bg-slate-900 text-white p-8 rounded-[3rem] shadow-xl">
                <span className="text-[10px] font-black text-purple-400 uppercase">{user.initialSoulAge}</span>
             ...
             <button onClick={() => { localStorage.clear(); window.location.reload(); }} className="w-full p-6 bg-red-50 text-red-600 rounded-[2rem] font-black uppercase text-xs">Reset Profile</button>
          </div>
        </div>
      )}
      ...
      {status === SafetyStatus.EMERGENCY && (
        <div className="fixed inset-0 z-[20000] flex flex-col p-8 text-white animate-fade-in">
           <div className="flex-grow flex flex-col items-center justify-center text-center space-y-10">
              ...
              <h1 className="text-7xl font-black uppercase">SOS</h1>
              <div className="bg-black/60 backdrop-blur-2xl rounded-[3rem] p-10 w-full text-left space-y-6 border border-white/20">
                 ...
                 <p className="text-3xl font-black text-red-400">Blood: {user.bloodGroup}</p>
              </div>
           </div>
